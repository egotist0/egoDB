...
func TestEncodeEntry(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"with-key-value", args{e: &LogEntry{Key: []byte("kv"), Value: []byte("egoDB"), ExpiredAt: 443434211}}, []byte{101, 208, 223, 156, 0, 4, 14, 198, 147, 242, 166, 3, 107, 118, 108, 111, 116, 117, 115, 100, 98}, 21,
		},
		{
			"type-delete", args{e: &LogEntry{Key: []byte("kv"), Value: []byte("egoDB"), ExpiredAt: 443434211, Type: TypeDelete}}, []byte{38, 27, 121, 27, 1, 4, 14, 198, 147, 242, 166, 3, 107, 118, 108, 111, 116, 117, 115, 100, 98}, 21,
		},
	}
...
func Test_getEntryCrc(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"normal", args{e: &LogEntry{Key: []byte("kv"), Value: []byte("egoDB")}, h: []byte{0, 4, 14, 198, 147, 242, 166, 3}}, 2631913573,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
